...
Class Action {
	...
	function login2(){
		...
		if($qry->num_rows > 0){
			...
			if($_SESSION['login_alumnus_id'] > 0){
				$bio = $this->db->query("SELECT * FROM Student where id = ".$_SESSION['login_alumnus_id']);
				if($bio->num_rows > 0){
	...
	function signup(){
		...
		$data .= ", username = '$email' ";
		$data .= ", password = '".md5($password)."' ";
		$chk = $this->db->query("SELECT * FROM  where username = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
		...
		if($save){
			...
			$save_alumni = $this->db->query("INSERT INTO Student set $data ");
			if($data){
	...
	function update_account(){
		...
		if($save){
			...
			$save_alumni = $this->db->query("UPDATE student set $data where id = '{$_SESSION['bio']['id']}' ");
			if($data){
				foreach ($_SESSION as $key => $value) {
					unset($_SESSION[$key]);
				}
	...
	function update_alumni_acc(){
		extract($_POST);
		$update = $this->db->query("UPDATE student set status = $status where id = $id");
		if($update)
			return 1;
	}
